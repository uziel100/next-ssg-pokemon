...
const PokemonPageDetail: NextPage<Props> = ({ pokemon }) => {
      ...
      <Container css={{ mt: "2rem" }}>
        <Grid.Container gap={2}>
          ...
          {/* <h1>
            ...
            <Image
              src={pokemon.sprites.back_default}
              alt={`Pokemon back ${pokemon.name}}`}
              width={200}
              height={200}
            />
            <Image
              src={pokemon.sprites.front_default}
              alt={`Pokemon front ${pokemon.name}}`}
              width={200}
              height={200}
            /> */}
        </Grid.Container>
...
export const getStaticPaths: GetStaticPaths = async (ctx) => {
  const pokemon150 = [
    ...new Array(150).fill(0).map((value, idx) => `${idx + 1}`),
  ];
  const pokemonPaths = pokemon150.map((id) => ({ params: { pokemonId: id } }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  return {
    props: {
      pokemon: await getPokemonInfo(pokemonId),
    },
  };
};
